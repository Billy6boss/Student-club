...
public class ClubExportPDF5 {
  ...
  //ClubMember data list
    private List<StuClass> sclist = null;
                ...
                if (max>0){
                   ...
                   // ------------------------------------------------- 
                    for (int i=0;i<max;i++){  
                    	...
                    	if(sclist.get(i).getStuBasis().getRgno() != checkrgno) {
                    		
                    	    checkrgno = sclist.get(i).getStuBasis().getRgno() ;   
                    	    String  checkclubname = ""; //社團
                    		String  cadrename = ""; // 職稱
                    		
                    		checkclubname = sclist.get(i).getClub().getClub_name();
                    		
                    		   
                    		    //(1) 此迴圈 確認 學生有多少職稱           		
                    		    for(int j=forruncount; j<sclist.size() ;j++) {                        			
                    			  if( checkrgno == sclist.get(j).getStuBasis().getRgno() ) {
                    				cadrenamecount += 1;
                    		      }
                    			}
                    		    
                    		    //(2) 依照現在學生 進行  社團比對 判斷相同 [社團]後 ，並對 [職稱] 進行 group
                    			for(int z=forruncount; z < (forruncount+cadrenamecount) ;z++) {   // z = 迴圈跑幾次 < (迴圈跑幾次+共有多少職稱) ;
                    				
                    				if(checkclubname == sclist.get(z).getClub().getClub_name()) {
                    				  if(forruncount == z) {
                    					if((sclist.get(z).getClubCadre().getCadre_name() == null) || sclist.get(z).getClubCadre().getCadre_name().equals("")) {
                    						cadrename += "社員";
                    					} else {
                    						cadrename += sclist.get(z).getClubCadre().getCadre_name();
                    					}                             		                                       		    
                    				  } else {  
                    					if((sclist.get(z).getClubCadre().getCadre_name() == null) || sclist.get(z).getClubCadre().getCadre_name().equals("")) {
                    						cadrename += "社員";
                    					...
                    					checkrgno = 0 ;  //  遇到 同樣學生(rgno)，但不同社團 就跳出。
                    				}
                        		}
                    			cadrenamecount = 0 ; // 完成 後清空 學生 職稱 總比數 

                    		datatable.addCell(this.addNewCell(sclist.get(i).getStuBasis().getRgno().toString(), f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    		datatable.addCell(this.addNewCell(sclist.get(i).getStuBasis().getCname(), f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    		datatable.addCell(this.addNewCell((sclist.get(i).getStuBasis().getSex() == 1) ? "男" : "女" , f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    		datatable.addCell(this.addNewCell(sclist.get(i).getClub().getClub_name(), f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    		datatable.addCell(this.addNewCell(cadrename, f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    	} else {
                	    	checkrgno = 0 ;  //跳過重複的 rgno ， 迴圈z裡已進行group 不需要再跑第2次
...
public void setHeaderString(String mat_cname, String adg_cname,String class_name , String dep_cname){
    String header =  this.title+" "+mat_cname+" "+ adg_cname +" "+ class_name + " 學生 " + dep_cname;
...
private PdfPTable setHeader()throws Exception{ 
    ...
    //step3:建立表格
    float headerwidths[] = {18,18,18,18,18};
  ...
  //step5:列印選
    	//座號
    	beanTable.addCell(this.addNewCell("座號", f_col, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0, 0.1f, 0.1f, 0.1f,0.1f));
        //姓名
        ...
        //參加社團
        beanTable.addCell(this.addNewCell("參加社團", f_col, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f));
        //職務
        beanTable.addCell(this.addNewCell("職務", f_col, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f));
        
    return beanTable;
}

public void setSclist(List<StuClass> sclist) {
	this.sclist = sclist;
}	
