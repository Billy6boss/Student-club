...
/**
 * 社團管理 : 報表系統 社團成員名單 Tom 創立日期 : 2018/04/23
 */
public class ClubExportPDF {
	...
	// public void setPrint_date(String Pvalue) {this.print_date = (Integer.parseInt(Pvalue.substring(0,4)))-1911 + Pvalue.substring(4);}
	// 列印日期
	...
	// set clib data list
	// public void setcmlist(List<ClubMember> cmlist) {this.cmlist = cmlist;};
	...
	public int Generate() throws Exception {
		...
		try {
			...
			int checkrgno = 0; // 確認學生是否重複
			int cadrenamecount = 0; // 一位學生的職稱數量
			int forruncount = 0; // 迴圈次數

			// 找出最大的筆數
			// max = (ec1.size()>ec2.size() == true) ? ec1.size() : ec2.size();
			int rowrk = 2;
			max = cmlist.size(); // cell 橫向欄位
			// if (max>0):表示有資料才列印
			if (max > 0) {
				...
				for (int i = 0; i < cmlist.size(); i++) {
					if (cmlist.get(i).getRgno() != checkrgno) {
						...
						datatable.addCell(this.addNewCell(String.valueOf(++tableno), f_content, Element.ALIGN_CENTER, 5,
								null, span_count, 0, 38, 0.1f, 0.1f, 0.1f, 0.1f));
						String cadrename = "";
						// (1) 此迴圈 確認 學生有多少職稱
						...
						datatable.addCell(this.addNewCell("", f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 38, 0.1f, 0.1f, 0.1f, 0.1f));
	...
	/** 建立儲存格 */
	private PdfPCell addNewCell(String pTxt, Font pFont, int pHor, int pPadding, Color pColor, int pRowspan, int pColspan, int pFixedH, float pTopW, float pBottomW, float pLeftW, float pRightW) {
		PdfPCell cell = null;
	...
	/**
	 * 設定標頭 (如:"OO高中 107學年度 下學期 「 籃球」 社團成員名單")
	 * @param mat_code
	 * @param adg_cname
	 * @param dep_cname
	 */
	...
	private PdfPTable setHeader() throws Exception {
		...
		// step3: 建立表格
		float headerwidths[] = { 4, 8, 10, 4, 10, 10, 24, 10, 10, 10 };
		Table tablep = new Table(10);
		...
		beanTable.addCell(this.addNewCell("序號", f_col, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
		...
		beanTable.addCell(this.addNewCell("備註", f_col, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0, 0.1f, 0.1f, 0.1f, 1f));
