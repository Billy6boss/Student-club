...
public class ClubExportPDF8 {
  ...
  //建立不同系所之頁碼陣列集合{5,1,8}表示共有三組系所組，總頁數分
    ...
    private List<ClubRecord> crlist = null;
 ...
 // method 
    ...
    public void setStuimgurl(String stuimgurl) {this.stuimgurl = stuimgurl;}
 ...
 */
    public int Generate()throws Exception{
       ...
       // CprogramDataList ec2= new CprogramDataList(); //選修
        ...
        // Step 2：建立版面配置所需要的參數
        int limit_count = 12; // 版面  限制每一頁可列印的筆數     +8 = page2 row
                ...
                if (max>0){
                   ...
                   // 課程科目迴圈------------------------------------------------- 
                    for (int i=0;i<max;i++){
                    		...
                    		datatable.addCell(this.addNewCell(crlist.get(i).getSbj_year().toString()+" "+((crlist.get(i).getSbj_sem() == '1') ? "上" : "下") , f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    		datatable.addCell(this.addNewCell(crlist.get(i).getClub().getClub_name() , f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));                  	
                    		datatable.addCell(this.addNewCell("起 : "+crlist.get(i).getCr_sdate()+" \n 訖 : "+crlist.get(i).getCr_edate() , f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    		datatable.addCell(this.addNewCell(crlist.get(i).getCr_detail() , f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));           	
                    		datatable.addCell(this.addNewCell(crlist.get(i).getCr_hours().toString() , f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                    		datatable.addCell(this.addNewCell("教師 : "+crlist.get(i).getStaff().getStaff_cname()+" \n "+crlist.get(i).getCr_time() , f_content, Element.ALIGN_CENTER, 5, null, span_count, 0, 0, 0.1f, 0.1f, 0.1f, 0.1f));
                        ...
                        //1:必修
                        beanData = ec1.get(j);
                        this.addCell2(beanData, datatable, border);
                        //2:選修
                        beanData = ec2.get(j);
                        this.addCell2(beanData, datatable, border);
                        */
...
private void setonTable()throws Exception{ 
    ...
    Color hcolor  = new java.awt.Color(255,255,255);//color
    	...
    	//圖片內容 
		//beanTable.addCell(this.addNewCell("IMG", f_col, Element.ALIGN_CENTER, 5, hcolor, 4, 1, 0, 0.1f, 0.1f, 0.1f,0.1f));
    
    	File file = new File(stuimgurl);  
    	if (file.exists() && file.canRead()) {
    		beanTable.addCell(headPng(stuimgurl, Element.ALIGN_CENTER, 5, hcolor, 4, 1, 0, 0.1f, 0.1f, 0.1f,0.1f));
    	}else {
    		beanTable.addCell(this.addNewCell("大頭貼", f_col2, Element.ALIGN_CENTER, 5, hcolor, 4, 1, 0, 0.1f, 0.1f, 0.1f,0.1f));
    	}
        
        float twoheaderwidths[] = {33,66};
    	...
    	//姓名欄位
        //cell = this.addNewCell("姓	名", f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 1, 0, 0.1f, 0.1f, 0.1f,0.1f);
        twobeanTable.addCell(this.addNewCell("姓	名", f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 1, 0, 0.1f, 0.1f, 0.1f,0.1f));  	
        //姓名
        //cell = this.addNewCell("李莫愁", f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 1, 0,  0.1f, 0.1f, 0.1f,0.1f);
        twobeanTable.addCell(this.addNewCell(crlist.get(0).getStuRegister().getStuBasis().getCname(), f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 1, 0,  0.1f, 0.1f, 0.1f,0.1f));
        //班級欄位
        twobeanTable.addCell(this.addNewCell("班	級", f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 0, 0, 0.1f, 0.1f, 0.1f,0.1f));
        //班級
        twobeanTable.addCell(this.addNewCell(crlist.get(0).getStuRegister().getStuClass().getCls_cname(), f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f)); 
        beanTable.addCell(twobeanTable);
        
        
        float thrheaderwidths[] = {49,49};
        ...
        //出生年月欄位 + 資料
        thrbeanTable.addCell(this.addNewCell("出生日期  : "+crlist.get(0).getStuRegister().getStuBasis().getBirthday().toString(), f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 1, 0,  0.1f, 0.1f, 0.1f,0.1f));             
        //學號欄位  + 資料
        thrbeanTable.addCell(this.addNewCell("學號  : "+crlist.get(0).getStuRegister().getStuBasis().getReg_no(), f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 1, 0,  0.1f, 0.1f, 0.1f,0.1f)); 
        //入學日期欄位  + 入學日期資料
        thrbeanTable.addCell(this.addNewCell("入學日期  : "+crlist.get(0).getStuRegister().getStuBasis().getCmat_year(), f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f)); 
        //性別欄位 + 性別代號  (1:男  , 2:女)
        thrbeanTable.addCell(this.addNewCell("性別  : "+((crlist.get(0).getStuRegister().getStuBasis().getSex() == 1) ? "男" : "女"), f_col2, Element.ALIGN_LEFT, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f));
...
private PdfPTable setunderTable()throws Exception{ 
    ...
    //step2:建立表格
    float headerwidths[] = {16,16,16,16,16,16};
    Table tablep = new Table(6);
    tablep.setWidths(headerwidths);
    tablep.getDefaultCell().setBorderColor(Color.BLACK);
    tablep.setBorderColor(Color.BLACK); // table 邊框線
    tablep.setConvert2pdfptable(true);//把Table轉回PdfPTable(Table預設外框為粗)  
    ...
    Color hcolor  = new java.awt.Color(255,255,255);//color
    	...
    	//圖片內容
		beanTable.addCell(this.addNewCell("學年學期", f_col2, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0, 0.1f, 0.1f, 0.1f,0.1f));
    	//姓名欄位
    	beanTable.addCell(this.addNewCell("參加社團", f_col2, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0, 0.1f, 0.1f, 0.1f,0.1f));
        //姓名
        beanTable.addCell(this.addNewCell("服務時間", f_col2, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f));
      //姓名
        beanTable.addCell(this.addNewCell("服務事項", f_col2, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f));
      //姓名
        beanTable.addCell(this.addNewCell("時數", f_col2, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f));
      //姓名
        beanTable.addCell(this.addNewCell("學校認證", f_col2, Element.ALIGN_CENTER, 5, hcolor, 0, 0, 0,  0.1f, 0.1f, 0.1f,0.1f));
        
      return beanTable;
}

public void setCrlist(List<ClubRecord> crlist) {
	this.crlist = crlist;
}
